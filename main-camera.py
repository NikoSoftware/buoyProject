...
USE_CAMERA = True  # 设置为True使用USB摄像头，False使用视频文件[7](@ref)
CAMERA_INDEX = 0  # USB摄像头设备索引，默认为0[1,8](@ref)
RESOLUTION = (1280, 720)  # 摄像头分辨率设置[4,6](@ref)
...
def main():
    ...
    if not cap.isOpened():
        if USE_CAMERA:
            ...
            print("可能原因: 1) 设备索引错误 2) 摄像头被其他程序占用 3) 驱动程序问题[6](@ref)")
        else:
